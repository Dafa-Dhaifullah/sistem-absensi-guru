                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($semuaPiket as $piket)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $piket->email }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('admin.akun-piket.edit', $piket->id) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                            
                                            <form action="{{ route('admin.akun-piket.resetPassword', $piket->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin reset password guru ini ke default?');">
                                                @csrf
