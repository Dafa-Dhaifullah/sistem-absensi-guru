                    ...
                    <form action="{{ route('admin.laporan.individu') }}" method="GET">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div class="md:col-span-2">
                                <x-input-label for="data_guru_id" :value="__('Pilih Guru')" />
                                <select id="data_guru_id" name="data_guru_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    ...
                                    @foreach ($semuaGuru as $guru)
                                        <option value="{{ $guru->id }}" 
                                            @if($request->data_guru_id == $guru->id) selected @endif>
                                            {{ $guru->nama_guru }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        ...
                        <div class="flex justify-between items-center mb-4">
                            <div>
                                <h3 class="text-lg font-medium">Hasil Laporan untuk: <span class="text-blue-600">{{ $guruTerpilih->nama_guru }}</span></h3>
